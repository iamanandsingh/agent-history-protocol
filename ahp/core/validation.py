...
def _validate_action(p: ActionPayload) -> List[str]:
    ...
    if len(p.tool_name) > MAX_TOOL_NAME_LENGTH:
        errors.append(f"tool_name too long: {len(p.tool_name)} > {MAX_TOOL_NAME_LENGTH}")
    if len(p.target_entity) > MAX_STRING_LENGTH:
        errors.append(f"target_entity too long: {len(p.target_entity)}")
    if len(p.evidence_uri) > MAX_STRING_LENGTH:
        errors.append(f"evidence_uri too long: {len(p.evidence_uri)}")
    if len(p.model_id) > MAX_TOOL_NAME_LENGTH:
        errors.append(f"model_id too long: {len(p.model_id)}")
