...
from ahp.core.chain import (
    ...
    parse_gap_payload, parse_checkpoint_payload, parse_recovery_payload,
    ...
    if rtype == RecordType.ACTION:
        ...
        for e in payload['authorization']['entries']:
            entry = {
                ...
                'authorizer_agent_id': uuid7_to_str(e['authorizer_agent_id']) if e['authorizer_agent_id'] != ZERO_UUID else None,
        ...
        result['payload'] = {
            'parent_action_id': uuid7_to_str(payload['parent_action_id']) if payload['parent_action_id'] != ZERO_UUID else None,
...
def format_action_summary(stored_bytes: bytes) -> dict:
    ...
    if auth_type == AuthorizationType.AUTH_NONE:
        ...
        auth_display = f"\U0001f464+\U0001f916 MULTI_PARTY"
    else:
        auth_display = "?"
