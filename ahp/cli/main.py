...
from ahp.core.types import (
    RecordType, AuthorizationType, ActionType, GapReason,
    ZERO_UUID,
)
from ahp.core.chain import (
    ChainReader, parse_envelope, parse_action_payload,
    parse_gap_payload, parse_witness_payload,
)
...
def cmd_log(chain: Optional[str] = None, last: Optional[int] = None,
            authorized_by: Optional[str] = None,
            unauthorized: bool = False) -> None:
...
def cmd_show(record_seq: int, chain: Optional[str] = None,
    ...
    if witness:
        ...
        if n == 0:
            ...
            for i, w in enumerate(witness_records):
                print(f"  [{i+1}] witness_id={w['witness_id']}, "
...
def cmd_trace(session_prefix: str, chain: Optional[str] = None) -> None:
    ...
    for stored in all_bytes:
        ...
        if session_str.startswith(session_prefix) or session_str.replace('-', '').startswith(session_prefix.replace('-', '')):
...
def cmd_init(agent_name: Optional[str] = None) -> None:
    ...
    if config_path.exists():
        print(f"ahp.yaml already exists. Remove it first to reinitialize.")
        sys.exit(1)
...
def cmd_keygen() -> None:
    ...
    print(f"  Public key:  ahp-key.pub")
    print(f"  Private key: ahp-key.priv")
...
def main() -> None:
    ...
    if cmd == 'log':
        cmd_log(chain=chain, last=last, authorized_by=authorized_by,
