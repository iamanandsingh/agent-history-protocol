...
class TestMapActionRecord(unittest.TestCase):
    ...
    def test_core_attributes(self):
        ...
        self.assertEqual(attrs['ahp.record.sequence']['intValue'],
                         str(self.record_json['sequence']))
        self.assertEqual(attrs['ahp.agent.id']['stringValue'],
                         self.record_json['agent_id'])
        self.assertEqual(attrs['ahp.session.id']['stringValue'],
                         self.record_json['session_id'])
...
class TestOTLPBatchFormat(unittest.TestCase):
    ...
    def test_each_log_record_has_required_fields(self):
        required_fields = [
            'timeUnixNano', 'severityNumber', 'severityText',
